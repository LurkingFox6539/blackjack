...
int makeDecision() {
	...
	if (players[1].handValue > 21) {
		...
		cout << "\n House has won the game, with a value of " << players[0].handValue << "!\n";
		return 3;
	}
	else if (players[0].handValue > players[1].handValue && players[0].handValue > 21) {
		cout << "\n House hand exceeds 21, House goes bust. Congrats player.\n";
		isWin = 1;
		return 4;
	}
	else if (players[0].handValue > players[1].handValue && players[0].handValue == 21) {
		...
		cout << "\n Player hit a Black Jack. Congrats player.\n";
		isWin = 1;
		return 6;
	}
	else if (players[0].handValue < players[1].handValue) {
		cout << "\n Player has beat House. Congrats Player.\n";
		isWin = 1;
		return 1;
	}
	else if (players[0].handValue == players[1].handValue) {
		cout << "\n Player and House are equal. Bets are returned.\n";
		return 2;
	}
}
